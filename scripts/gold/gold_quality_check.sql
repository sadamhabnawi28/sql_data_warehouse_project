...
HAVING COUNT(*) > 1; -- result = Null --> Customer key is unique
...
HAVING COUNT(*) > 1; -- result = Null --> Product key is unique
...
WHERE p.product_key IS NULL 
   OR c.customer_key IS NULL; -- result = Null --> Every fact Row has valid dimension keys
